...
function DataTable({
  ...
  const getValue = (row, accessorKey) => {
          ...
          <thead className="bg-muted/50">
            <tr className="border-b transition-colors hover:bg-muted/80">
              {columns.map((column, i) => (
                <th key={i} className="p-2 md:p-4 text-left font-medium text-muted-foreground text-xs md:text-sm whitespace-nowrap">
                  {column.header}
                </th>
              ))}
              {actions && <th className="p-2 md:p-4"></th>}
            </tr>
          </thead>
          <tbody>
              ...
              paginatedData.map((row, rowIndex) => (
                ...
                >
                  {columns.map((column, colIndex) => (
                    <td key={colIndex} className="p-2 md:p-4 text-xs md:text-sm">
                      {column.cell
                        ? column.cell({ 
                            row, 
